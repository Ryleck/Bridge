...
router.get('/', function(req, res, next) {
    ...
    res.render('table', {title: 'Table ' + req.query.table, feuille: resumeTable(req.query.table, req.query.ronde, req.query.genere, req)});
...
function resumeTable(table, ronde, genere, req){
    ...
    if(genere === '1') {
        ...
        if(nbPpR > 1) {
            for (i = parseInt(1 + nbPpR * (parseInt(ronde) - 2)); i <= parseInt(nbPpR * (parseInt(ronde) - 1)); i++) {
                //console.log('**** ' + req.query.pNS.length);
                varPl.setScore(table, i, (parseInt(req.query.pNS[j]) - parseInt(req.query.pEO[j])));
                j++;
            }
        } else {
            varPl.setScore(table, parseInt(1 + nbPpR * (parseInt(ronde) - 2)), (parseInt(req.query.pNS) - parseInt(req.query.pEO)));
        }
    ...
    if(parseInt(ronde) < parseInt(nbR)){
        htmlFormS = htmlFormS + '<form action="table" method="get"><input type="hidden" name="genere" value="0"><input type="hidden" name="ronde" value="' + rondeInt + '"><input type="hidden" name="table" value="'+table+'">';
        htmlFormE = '</tbody></table><input type="submit" value="Ronde suivante"></form></div>';
    }else{
        htmlFormS = htmlFormS + '<form action="terminer" method="get"><input type="hidden" name="table" value="'+table+'">';
        htmlFormE = '</tbody></table><input type="submit" value="Terminer"></form></div>';
    }
    ...
    var retour = '<div id="divPl">' +
        htmlFormS +
        '<table id="tablePointage"><tbody><tr><td id="contrat" rowspan="2">&Eacute;tui</td><td id="contrat" colspan="2">&Eacute;quipes</td>'+
        '<td id="contrat" rowspan="2">Contrat</td><td id="contrat" rowspan="2">Par</td><td id="contrat" rowspan="2">Entame</td><td id="contrat" rowspan="2">R&eacute;sultat</td>'+
        '<td id="contrat" colspan="2">Points</td></tr><tr><td id="contrat">N-S</td><td id="contrat">E-O</td><td id="contrat">N-S</td><td id="contrat">E-O</td></tr>';
...
function genetePointagePlanchette(pl, equipeNS, equipeEO){
    ...
    sPl = sPl + '<tr>' +
        ...
        '<td id="contrat"><select name="nbLeves">' +
        ...
        '</select>' +
        '</td>' +
        '<td id="pointageNS"><input class="txtPoints" type="text" name="pNS" value="0"></td>' +
        '<td id="pointageEO"><input class="txtPoints" type="text" name="pEO" value="0"></td></tr>';
